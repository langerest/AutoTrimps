...
function updateAutoMapsStatus(get) {
    ...
    //Spire
        ...
        status = 'Void Maps: ' + game.global.totalVoidMaps + ((stackedMaps) ? " (" + stackedMaps + (stackedMaps == 1 ? " stack)" : " stacks)") : "") + ' remaining';
    ...
    if (Object.keys(mapSpecialModifierConfig).forEach(function(o) {
        ...
        if (c) {
            if (59 <= game.global.highestLevelCleared) {
                if (needPrestige && a.includes("p") && (challengeActive("Mapology") || getPageSetting("PrestigeOverFarm"))) {
                    c.value = "p";
